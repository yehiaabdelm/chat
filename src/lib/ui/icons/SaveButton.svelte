<script lang="ts">
	...
	let {
		...
		hovered = false,
		onclick
	}: { active: boolean; hovered: boolean; onclick?: () => void } = $props();
	let grey = 245;
	...
	let aFill = new Spring(active ? 0.542 : 0, { stiffness: 0.1, damping: 0.8 });
	let aStroke = new Spring(0, { stiffness: 0.1, damping: 0.8 });
	$effect(() => {
		if (active) {
			aFill.set(1);
			r.set(180);
...
<TextIcon children={icon} initialOpacity={0.9} text="" {onclick} />
